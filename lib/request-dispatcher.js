...
class RequestQueue {
  ...
  async traverse() {
    if (!this.busy) {
      this.busy = true;
      while (!this.isEmpty()) {
        const {url, ...options} = this.peek();
        await backOffFetch(url, options);
        this.requests.shift();
      }
      this.busy = false;
    }
...
async function backOffFetch(url, options, wait = INITIAL_WAIT, attempt = 0) {
  try {
    ...
    if (wait < MAX_RETRY) {
      console.log(`Retrying in ${wait}ms, attempt ${attempt} failed error= ${e.message}`);
      await new Promise(r => setTimeout(r, wait));
      wait = (wait * BACKOFF) + Math.floor(Math.random() * 100); // update retry interval
      await this.send(url, options, wait, ++attempt);
    } else {
