...
import { REQUEST_BACKOFF, REQUEST_INITIAL_RETRY_WAIT, REQUEST_MAX_RETRY } from './environment';
 ...
 */
async function backOffFetch(url, options, wait = REQUEST_INITIAL_RETRY_WAIT, attempt = 0) {
  await fetch(url, options).then(async (res) => {
    if (!res.ok) {
      console.log(`Failed to send request ${options.method} to ${url}`);
      if (wait < REQUEST_MAX_RETRY) {
        console.log(`Retrying in ${wait}ms, attempt ${attempt + 1} failed`);
        await new Promise(r => setTimeout(r, wait));
        wait = (wait * REQUEST_BACKOFF) + Math.floor(Math.random() * 100); // update retry interval
        await backOffFetch(url, options, wait, ++attempt);
      } else {
        console.log(`Max retry off ${REQUEST_MAX_RETRY}ms was reached, delta message was lost ...`);
      }
    } else {
